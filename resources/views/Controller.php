...
use TCG\Voyager\Http\Controllers\ContentTypes\MultipleFile;
...
abstract class Controller extends BaseController
{
    ...
    public function insertUpdateData($request, $slug, $rows, $data)
    {
        ...
        foreach ($rows as $row) {
             ...
             */
            if (($row->type == 'multiple_images' or $row->type == 'multiple_files') && !is_null($content)) {
            ...
            if (is_null($content)) {
                ...
                // If the multiple_files upload is null and it has a current file keep the current file
                if ($row->type == 'multiple_files' && is_null($request->input($row->field)) && isset($data->{$row->field})) {
                    $content = $data->{$row->field};
                }
